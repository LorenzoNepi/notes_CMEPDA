...
\textbf{Error handling} is one of the most important problems to solve when designing a program. What should you do when a piece of code fails? What does fail mean? Here are some examples:
\begin{itemize}
    ...
    \item[-] Output cannot be found in a reasonable amount of time
    \item[-] Runtime resource failures: network connection down, disk space exhausted\dots
\end{itemize}
...
A typical convention for programs is to return 0 from the main if the execution was successful and an error code (integer number) otherwise. However this isn't the best practice. Take a look at this example:

\input{pygments/error_flags}

Error codes have their use and they are fine in some cases, but they suffer from a few issues:
\begin{itemize}
...
\begin{itemize}
    ...
    \item[-] is easy to report to upper levels without lots of lines of code.
\end{itemize}
...
Even worse, you should never catch  \texttt{BaseException} as that would even prevent the user from aborting the execution with a \texttt{KeyboardInterrupt} (e.g. Ctrl-C). 
...
Here is how you can extract and log a traceback using the \texttt{logging} module:
\input{pygments/traceback_logging}

\subsubsection{- Traceback report with \texttt{traceback}}
Alternatively, you can use the \texttt{traceback} module directly:
\input{pygments/traceback_tb}

\subsubsection{- Exception logging with \texttt{traceback}}
You can also log the exception details directly as shown below:
...
One last question: when should you catch exceptions? Unlike error flags, which need to be checked as early as possible, you are not in a rush with exceptions. Remember: your goal is to provide the user a meaningful error message and useful debug information. You should catch an exception only when you have enough context to do that, which sometimes means waiting a few levels in the hierarchy. Here's a long example to let you understand this concept:
...
This first snippet demonstrates what happens when you catch an exception too early, losing valuable context:
\input{pygments/when_to_catch_1}

\hrulefill

\subsubsection{Catch when needed}
This second snippet demonstrates the proper approach, catching the exception only when you have enough context to handle it gracefully:
...
Here is an example of a basic custom iterator:
...
And here is a more eccentric example of an iterator to show its flexibility:
...
We have seen that iterators are useful to iterate over containers. However, that assumes a container exists, which means slots of memory are being used.
\textbf{Generators} allow you to loop over sequences of items even when they don't exist beforehand - the items are just created \textbf{lazily} the moment they are required. For example you can write a generator to loop over the Fibonacci succession. You can't create the sequence earlier, since it is not finite.
...
A \textbf{generator function} is a function that contains the keyword \texttt{yield} at least once in its body. When you call a generator function the code is not executed - instead a generator object is created and returned (even if you don't have a return statement). Each call to \texttt{next()} on the returned generator will make the function code run until it finds a \texttt{yield} statement. Then the execution is paused and the value of the expression on the right of \texttt{yield} is returned (yielded) to the caller. A further call of \texttt{next} will resume the execution from where it was suspended until the next \texttt{yield} and so on. Eventually, when the function body ends, \texttt{StopIteration} is raised. Usually generator functions contain a loop, but it's not mandatory.
Here are two examples of generator functions:

\textbf{- Simple generator function}

This is a straightforward implementation of a generator function:
\input{pygments/generator_functions}

\textbf{- Infinite sequence generators}

Generators truly shine when dealing with infinite sequences, such as the Fibonacci numbers:
...
As a brief digression, let's talk about \textbf{Anonymous functions}, or \textbf{lambda functions}, which are a construct typical of \textbf{functional programming} (see for example \url{https://en.wikipedia.org/wiki/Lambda_calculus} and \url{https://en.wikipedia.org/wiki/Functional_programming}).
...
Here is the first simple implementation:
\input{pygments/file_iterator}
\bigskip

\textbf{- File iterator redone}

We can rewrite this more elegantly using a generator function:
\input{pygments/file_iterator_2}
\bigskip

\textbf{- File iterator, final version}

Finally, here is the most Pythonic version using a generator expression:
