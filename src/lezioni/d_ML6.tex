...
Graph Neural Networks were theorized long before the machine learning revolution that saw them take center stage. The beginning of their rapid rise in both the international market and academic research can be traced back to the 2018 paper available at this link: \url{https://arxiv.org/abs/1806.01261}.
...
Recently GNNs were popularized by Google DeepMind for their weather forecasting model, which obtained similar or better results than state-of-the-art non-ML models, but with less run time.
...
The raw input data typically resides in a physical space $X$, where features might represent concrete quantities like spatial coordinates or velocities. Let $x_i$ be the input vector of the node $v_i$ (e.g. position, velocity, mass of a celestial body if your graph is representing an astrophysical system). Embedding consists of mapping $x_i$ (the physical data) into a higher dimensional vector 
\begin{equation}
...
\begin{itemize}
    ...
    \begin{equation}
        e'_k = \phi^e(e_k, h_{r_k}, h_{s_k}, u)
    \end{equation}
    \item \textbf{Aggregation:} A single node may receive multiple messages from various neighbors, since they might be connected to more than one edge. To process them, the network uses an \textbf{aggregator}, which is a permutation-invariant \textbf{pooling function} $\rho^{e \to v}$ (such as an element-wise sum, a max or a mean). This step effectively compresses the set of incoming message vectors ($E'_i$) into a single unified vector $\bar{e}'_i$ of the same dimensionality:
    \begin{equation}
        \bar{e}'_i = \rho^{e \to v}(E'_i)
    \end{equation}
    \item \textbf{Node Update:} The node must now synthesize its own current state with the summarized information from its neighbors. A node neural network $\phi^v$ takes the aggregated message $\bar{e}'_i$ and the node's current state $h_i$ to output the newly updated latent state $h'_i$:
    \begin{equation}\label{eq:node_update}
        h'_i = \phi^v(\bar{e}'_i, h_i, u)
    \end{equation}
\end{itemize}
...
At the LHC, algorithms must quickly determine the origin of particle jets (e.g., identifying if a jet originated from a bottom quark, charm quark, or light quark). Because jets are point clouds, GNN architectures like the CMS \textit{ParticleNet} and the ATLAS \textit{GN1/GN2} models have superseded older sequence-based models (like RNNs) and CNNs. These graph networks achieve dramatic improvements, such as a $\sim 5\times$ better background rejection rate when isolating specific decay channels.
