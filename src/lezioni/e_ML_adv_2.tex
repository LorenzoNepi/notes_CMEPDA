...
\end{figure}
\vspace{-7.5mm}
With that said, it should be clear that CAEs are incredibly effective for unsupervised feature extraction. For example, take a look at the following study: \url{https://ieeexplore.ieee.org/document/7412749}. Here, Convolutional Sparse Autoencoders (CSAEs) (a kind of CAE that introduces a sparsity penalty to force  most neurons to remain inactive for every given input) have been successfully used in medical imaging to extract features for breast density segmentation and mammographic risk scoring. The unsupervised CSAE isolates dense tissue structures without requiring pixel-perfect manual annotations, providing a foundation for subsequent supervised classifiers.
...
This post-hoc explainability analysis perfectly demonstrated that without prior semantic segmentation, a CNN can easily "cheat" by learning confounding variables. By using a U-Net to strictly isolate the lungs and remove the irrelevant background, we force the subsequent classifier to base its predictions solely on genuine anatomical and pathological features.
...
\section{Hands-on example on U-Nets}
...
\end{itemize}
Here is a general scheme of the project, which you can find here in full: \url{https://colab.research.google.com/drive/1iPq8FnB_XvkirySFTQbIynOEhNcjXNFg?usp=sharing}

\begin{center}
    \includegraphics[width=0.6\linewidth]{figuresML_adv/lesson_ml_adv_2_pag_56.png}
\end{center}

Since I have to start a new page in order to input the Notebook in my LaTeX file, here is a photo of McEnroe and Borg before the 1980 Wimbledon final to occupy the rest of the page.
