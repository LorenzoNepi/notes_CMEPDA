...
This additional step makes development slightly slower, but the resulting execution is significantly faster. Moreover, while Python reveals syntax errors only when it tries to execute the flawed line, the C compiler will catch all syntax errors beforehand, completely preventing compilation until they are fixed.

Let's now take a look at a simple program in C and go through  its execution.

\subsection{The anatomy of a basic C program}

Here it is: the classic "Hello World" program. 

\input{pygments/c_hello_world}
...
\input{pygments/c_type_1}

\subsubsection{- Booleans}

Booleans are not defined in plain C. You must \texttt{\#include <stdbool.h>} to have the \texttt{bool} type defined. Otherwise, you can use the smallest integer (\texttt{char}) to represent them as 1 (true) and 0 (false). Here is how you can handle booleans in C:

\input{pygments/c_type_2}
...
\input{pygments/c_type_3}

\hrulefill

\subsubsection{Note: Variables vs. Literals} 
The variables defined in the previous piece of code seem very similar to what we have seen in the paragraph about "Integers and Floating Point numbers", don't they? 
...
When in doubt, it is always  best practice to use parentheses \texttt{()} to explicitly define the execution order and make your code significantly more readable. The following table shows all the operators in order of precedence (from top to bottom).
...
\input{pygments/c_control_conditionals}
...
\input{pygments/c_control_loops}
...
\input{pygments/c_strings_and_math}
...
\input{pygments/c_typedef_structs}
