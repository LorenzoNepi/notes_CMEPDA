...
Pay attention: even if it is called "flow model", $u_t^\theta$ is actually a vector field  with parameters $\theta$ (by our previous definitions) which takes a data point and the corresponding timestamp and returns a velocity ($\mathbb{R}^d \times [0,1] \rightarrow \mathbb{R}^d$). 
...
It is fundamental to notice that in this case, \textit{the term "latent space" is just a semantic relic from older models}: in reality the 2 distributions live in the exact same space and with latent space we can refer to the cloud of starting points. \\ 
...
This movement (schematically represented in figure) means solving the "right" ODE defined by the "right" vector field, i.e. our NN.
...
While ODEs are used to define flow models, another powerful class of equations called \textbf{Stochastic Differential Equations (SDEs)} forms the mathematical backbone of another ML paradigm: \textbf{diffusion models}. The general form of a It\^o SDE is given by:
\begin{equation}
...
Given the randomness of diffusion models, with such algorithms it is easier to explore the space of solutions. However, experts are slowly realizing that flow models can be just as expressive as their stochastic counterpart.
...
\end{equation}
If we then sample $x$ from this prob. path we get points distributed in this way:
\begin{equation}
...
With what we said since now, understanding the meaning of these next definitions should come natural. \\
...
Just like before, a pivotal example is that of a \textbf{gaussian conditional vector field}, which has the following form (you can easily find the explicit computation in literature):
\begin{equation}
...
Following the same logic as before (we want $P_{init}$ to go in $P_{data}$ not in a single point etc...) we analogously define the \textbf{marginal vector field} $u_t^{target}(x)$:
\begin{equation}
\begin{aligned}
...
\end{equation}
Even it is computable this seems apparently useless for our purposes. However the following theorem solves all of our doubts: $\mathcal{L}_{CFM}(\theta)$ and $\mathcal{L}_{FM}(\theta)$ are exactly equal up to a \textbf{constant} $C \in \mathbb{R}$ which does not depend on the parameters $\theta$, i.e.
\begin{equation}
...
\end{align*}
We have to show that $$\mathbb{E}[u_t^\theta(x)^T u_t^{target}(x)] = \mathbb{E}[u_t^\theta(x)^T u_t^{target}(x|y)]$$ in order to prove that $$\nabla_\theta \mathcal{L}_{FM}(\theta) = \nabla_\theta \mathcal{L}_{CFM}(\theta)$$ In fact, those are the only two terms inside the exp. value operator that are different and depend on $\theta$. By definition of expected value we have:
\begin{align*}
