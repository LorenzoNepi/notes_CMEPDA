\section{Introduction to Deep Learning and Artificial Neural Networks}

\subsection{Introduction}\label{sec:ML_images}
As mentioned at the beginning of the chapter, deep learning is machine learning made with Deep Neural Networks. \\ Before starting to dive deeper in DNN, let's talk for a second about digital images. It should be clear that a 2D Grayscale image is a 2D matrix, so it's a rank-2 tensor with shape (height, width). Instead, an RGB image is a 3D tensor with shape (height, width, channels), where $I(x, y) = [R(x, y), G(x, y), B(x, y)]$. There also exist 3D Grayscale images, which can be seen as a rank-3 tensor with shape (height, width, depth). They are vastly used in medicine, e.g in computed tomography.
...
Let's try to ease into DL starting from the concepts we are familiar with. Let's say for example that we want to write an algorithm which recognizes if there is a cat in a given image. In our example (figure below) a 64x64 image can be flattened into an input vector with components $x_1$ to $x_n$. 
We can apply a linear transformation $wx + b$ followed by a sigmoid function $\sigma$, such that $\hat{y} = \sigma(\theta^T x) = \sigma(wx + b)$. Note that we have changed the notation just a little: the parameters are now called $w$ and not $\theta$ just because in DNN we usually refer to them as \textbf{weights}.  
...
\end{equation} 
Notice that this is not yet a neural network since each output is totally independent of one another. 

\begin{center}
\includegraphics[width=0.75\textwidth]{figuresML/lesson_ml_1_pag_82.png}
\end{center}

Let's change one more time the problem by introducing the constraint that there is strictly a cat or a dog or a bird. It will be useful to change our notation slightly, as shown in the figure below. We isolate the linear part and write it as such: $z_1^{[1]} = w_1^{[1]}x + b_1^{[1]}$.  We also use the \textbf{softmax} function as our activation function:
\begin{equation}
...
\end{equation}
Since there is a relationship between all the outputs, this is the first shallow example of a neural network.
...
Now, the learning scheme is quite the same of the case of linear regression we have seen at the beginning of the chapter: at a certain point we will find a loss function and use the gradient descent method to minimize it. The difference is that before doing this, we need to pass through all the structure of our neural network. The process works through \textbf{forward propagation}: we process the input data according to the operation of the neuron sequentially (i.e. the last activation will be a function of the last $z$, which is a function of the previous activation, and so on), as shown below. 

\begin{center}
\includegraphics[width=0.75\textwidth]{figuresML/lesson_ml_1_pag_93.png}
\end{center}

Once we have computed the activation of the last layer, we have our prediction so we can compute the value of the loss function. To minimize this loss function using an optimization algorithm like Stochastic Gradient Descent, we need to \textit{compute the derivatives of the loss function with respect to all the network weights}. 

As an example, let's try to do it explicitly for our neural network. Since it has only one neuron in the last layer, we are obviously trying to solve a binary classification task, so we will use the binary cross entropy as a loss function.\\
To clarify the notation:
\begin{itemize}
    \item[-] With \textit{cost function} we mean all the loss functions computed on a dataset ($m$ is the batch size): $J(\hat{y}, y) = \frac{1}{m}\sum_{i=1}^{m}L^{(i)}(\hat{y}, y)$.
    \item[-] With \textit{loss function} we mean the single error computation, e.g., binary cross-entropy: \\ $L^{(i)} = -[y^{(i)}\log\hat{y}^{(i)} + (1-y^{(i)})\log(1-\hat{y}^{(i)})]$.
\end{itemize}
...
The most common NN in the '90s was the \textbf{Multi Layer Perceptron} (MLP). 
...
Let's now take a step back and ask ourselves: \textit{why do we need activation functions?} The "horse" problem shown below illustrates what happens if we do not use an activation function in a Neural Network i.e., if we do not introduce non-linearity. If we train a linear classifier on images of horses facing left or right, the learned weights look like a two-headed horse. There are no two-headed horses! \textit{Non-linearity is required}.
...
\begin{itemize}
    \item[-] \textit{Linear}: $f(x) = x$. Of course it cannot be used in hidden layers (where non linearity is mandatory) but it can be useful for output nodes in regression problems if the output values span in a very long range.
    \item[-] \textit{Sigmoid}: $f(x) = \sigma(x) = \frac{1}{1+e^{-x}}$. It was used historically in hidden layers but leads to an optimization problem called \textit{vanishing gradient}. It can be useful in the last layer for binary classification problems with a single output or multi-label classifiers.
    \item[-] \textit{Softmax}: $f_i(\vec{x}) = \frac{e^{x_i}}{\sum_{j=1}^{J}e^{x_j}}$. As seen, it useful in classification problems with one-hot encoded multi-class labels.
    \item[-] \textit{Rectified Linear Unit (ReLU)}: $f(x) = \max(0, x)$. It is usually the default for hidden layer activations (we will soon see why). Variants of ReLU include Leaky ReLU, Swish and Exponential Linear Unit (ELU).
    \item[-] \textit{tanh}: It is not very used anymore even if it was historically relevant.
...
It should be clear now that the choice of the activation function for the output layer is pivotal. Here we sum up the correct activation function, loss function and also output label for the three kinds of simple problems we have faced:
\begin{itemize}
    \item[-] \textit{Regression}: Since we want to have a continuous output, we use a linear activation. Loss: Mean Squared Error (MSE) or Mean Absolute Error (MAE). Label: continuous numbers.
    \item[-] \textit{Binary Classification}: We use a Sigmoid activation. Loss: Binary Cross-Entropy. Label: 0 or 1.
    \item[-] \textit{Multi-class Classification}: We use a Softmax activation. Loss: Categorical Cross-Entropy (or Sparse Categorical Cross-Entropy if labels are integers). Label: one-hot for CCE, integers for SCCE (0, 1, 2, 3, ...). Note that it is always better to use one-hot encoding for multi-class classification.
\end{itemize}

\subsection{A deeper look at overfitting and regularization}
As discussed previously, if the capacity is large enough the network could overfit on the training dataset. We must have a separate, statistically independent, validation/generalization sample to evaluate performance (with loss or other metrics). it should also be noticed that training results depend on many choices like the size of the data batches (usually limited by the memory of your GPU), learning rate (how much you move along the gradient at each iteration), chosen gradient descent algorithm, and capacity of the network. Take a look at the following figures to see what could really happen at your loss function during training (there is noise, loss that dramatically increases at a certain point without a clear reason and much more).

\begin{center}
\includegraphics[width=0.65\textwidth]{figuresML/lesson_ml_1_pag_124.png}
\end{center}

Previously, we saw that adding a penalty to the loss function induces a regularization. 
...
\textbf{Keras} is a Python library that allows to build, train, and evaluate NN with many modern technologies. Nowadays the most used library is \texttt{torch} because GPT, transformer and more complex architecture are written with it, because it seems to be more efficient than \texttt{tensorflow} (which Keras is part of). However in this kind of problems it is very hard to measure the actual efficiency (we will see why). 
...
The \textit{functional API} allows to express the idea that each layer is evaluated on the output of a previous layer, as shown here. Take a look at the \texttt{summary} of the NN, which explains its structure.

\input{pygments/keras_2}

\subsubsection{- Training a model with Keras}

During training, some callbacks can be passed to the fit function to monitor progress or adapt the training (e.g. stop if there aren't improvements in the last N epochs, change learning rate if there aren't improvements in M epochs, etc...). Some callbacks are predefined in Keras (like \texttt{EarlyStopping}, \texttt{ReduceLROnPlateau}), while others can be user-implemented. \\ 
